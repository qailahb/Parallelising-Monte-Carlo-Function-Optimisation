...
class MonteCarloMinimization {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < num_searches; i++) {
      ...
      if ((!searches[i].isStopped()) && (local_min < min)) { // don't look at those who stopped because hit exisiting
                                                             // path
        min = local_min;
      ...
      if (DEBUG)
        System.out
            .println("Search " + searches[i].getID() + " finished at  " + local_min + " in " + searches[i].getSteps());
    }
    // end timer
    tock();
