...
public class MonteCarloMinimizationParallel extends RecursiveTask<Integer> {
  ...
  private static int totalNumSearches = 0; // Global total number of searches to be performed
  private int numSearches = 0; // Total number of searches to be performed
  ...
  public MonteCarloMinimizationParallel(int startRow, int endRow, int rows, int columns, double xmin, double xmax,
    ...
    numSearches += (int) ((endRow - startRow + 1) * columns * searchesDensity);
    totalNumSearches += numSearches;
    // this.terrain = new TerrainArea(endRow-startRow+1, columns, xmin, xmax, ymin,
    // ymax);
  ...
  protected Integer compute() {
    if (endRow - startRow <= THRESHOLD) {
      ...
      //System.out.printf("startRow: %d, mid: %d, endRow: %d\n", startRow, mid, endRow);
      // Creates new tasks for each half of range
      MonteCarloMinimizationParallel leftTask = new MonteCarloMinimizationParallel(startRow, mid, rows, columns, xmin,
          xmax, ymin, ymax, searchesDensity);
      MonteCarloMinimizationParallel rightTask = new MonteCarloMinimizationParallel(mid, endRow, rows, columns, xmin,
          xmax, ymin, ymax, searchesDensity);
  ...
  public static void main(String[] args) {
    ...
    MonteCarloMinimizationParallel parallelMC = new MonteCarloMinimizationParallel(0, rows - 1, rows, columns, xmin,
        xmax, ymin, ymax, searchesDensity);
    ...
    System.out.printf("\t Search density: %f (%d searches)\n", searchesDensity, totalNumSearches);
